...
export default function PromoPage() {
  ...
  const [itemsPerPage, setItemsPerPage] = useState(12) // Changed to 12 for better grid layout
  ...
  // Promo Card Component for Grid View
  const PromoCard: React.FC<{ promo: PromoItem }> = ({ promo }) => {
      ...
      <div className="bg-white dark:bg-dark-surface rounded-lg shadow-lg border border-champagne-200 dark:border-dark-border overflow-hidden hover:shadow-xl transition-shadow duration-300">
        ...
        <div className="relative h-48 bg-gradient-to-br from-asparagus-50 to-champagne-100 dark:from-dark-bg dark:to-dark-surface">
          {imageUrl && !imageError ? (
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center p-4">
                <svg className="h-16 w-16 mx-auto text-gray-300 dark:text-gray-600 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="absolute bottom-2 left-2">
            <Badge variant="secondary" size="sm" className="shadow-md bg-white/90 dark:bg-dark-surface/90">
              {promo.brand}
            </Badge>
          </div>
        </div>
        ...
        <div className="p-4">
          ...
          {/* Promo Name */}
          <h3 className="text-lg font-semibold text-jet-800 dark:text-white mb-2 line-clamp-2 min-h-[3.5rem]">
            {promo.nama}
          </h3>

          {/* Promo Value */}
          <div className="mb-3">
            <div className="text-2xl font-bold text-asparagus-700 dark:text-asparagus-400">
              {formatCurrency(promo.nilai)}
            </div>
            <div className="text-xs text-gray-500 dark:text-gray-400">Minimum Purchase</div>
          </div>
          ...
          <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400 mb-3">
            ...
            <span>Expires: {formatDate(promo.expired)}</span>
          </div>
          ...
          <div className="flex items-center justify-between pt-3 border-t border-gray-200 dark:border-dark-border">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="font-medium text-jet-700 dark:text-gray-300">
                {promo.free_items?.length || 0} Free Items
              </span>
            </div>
            ...
            <Button
              ...
              size="sm"
              variant={expandedRows.has(promo.kode) ? "primary" : "outline"}
              className="text-xs"
            >
              {expandedRows.has(promo.kode) ? 'Hide' : 'View'}
            </Button>
          </div>
                  ...
                  <div className="flex items-center gap-2 bg-gray-100 dark:bg-dark-bg rounded-lg p-1">
                    <button
                      ...
                      className={`px-3 py-2 rounded-md transition-colors ${
                        viewMode === 'grid'
                          ? 'bg-white dark:bg-dark-surface text-asparagus-700 dark:text-asparagus-400 shadow-sm'
                          : 'text-gray-600 dark:text-gray-400 hover:text-asparagus-600 dark:hover:text-asparagus-400'
                      }`}
                      title="Grid View"
                    >
